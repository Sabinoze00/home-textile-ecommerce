...
export function useSearchAutocomplete(query: string, options: { enabled?: boolean } = {}) {
  const { enabled = true } = options
  ...
  const debouncedGetSuggestions = useRef(
    debounce(async (searchQuery: string) => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json()
          const productSuggestions = data.results.map((item: SearchResult) => item.name)
          const categorySuggestions = data.categories.map((item: SearchSuggestion) => item.name)
